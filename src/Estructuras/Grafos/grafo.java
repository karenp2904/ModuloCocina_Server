...
public class grafo {
    ...
    private static ArrayList<String> buscarRuta(String direccionOrigen, String direccionDestino) {
        ArrayList<String> ruta = new ArrayList<>();
        HashMap<String, Boolean> visitados = new HashMap<>();
        HashMap<String, String> padres = new HashMap<>();

        for (String direccion : grafo.keySet()) {
            visitados.put(direccion, false);
        }

        visitados.put(direccionOrigen, true);
        ...
        while (!cola.isEmpty()) {
            ...
            if (direccionActual.equals(direccionDestino)) {
                // Se encontró la dirección de destino, construir la ruta
                String direccion = direccionDestino;
                while (direccion != null) {
                    ruta.add(0, direccion);
                    direccion = padres.get(direccion);
                }
                break;
            }

            ArrayList<String> vecinos = grafo.get(direccionActual);
            for (String vecino : vecinos) {
                if (!visitados.get(vecino)) {
                    visitados.put(vecino, true);
                    padres.put(vecino, direccionActual);
                    cola.add(vecino);
                }
            }
        }
    ...
    public static void imprimirRuta(String direccionOrigen, String direccionDestino) {
        ArrayList<String> ruta = buscarRuta(direccionOrigen, direccionDestino);

        if (ruta.isEmpty()) {
            System.out.println("No se encontró una ruta entre las direcciones especificadas.");
        } else {
            System.out.println("Ruta encontrada:");
            for (String direccion : ruta) {
